package page.stephens.bountywell.data.remote
...
const val SYNC_BASE_URL: String = "https://bountywell.com/api"
...
class SyncApi(
    ...
    suspend fun putData(payload: JsonElement): String {
        val body = BountywellJson.encodeToString(JsonElement.serializer(), payload)
...
fun createSyncHttpClient(tokenStore: TokenStore, baseUrl: String = SYNC_BASE_URL): HttpClient =
    HttpClient {
        expectSuccess = false
        install(ContentNegotiation) { json(BountywellJson) }
        install(Auth) {
            bearer {
