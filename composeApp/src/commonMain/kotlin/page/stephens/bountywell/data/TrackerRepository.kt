...
import page.stephens.bountywell.data.model.BountywellJson
import page.stephens.bountywell.data.model.Profile
import page.stephens.bountywell.data.model.SyncBlob
...
class TrackerRepository(
    ...
    private val json: Json = BountywellJson,
    private val dispatcher: CoroutineDispatcher = Dispatchers.Default,
) {
    private val mutex = Mutex()
    private var database: BountywellDb? = null

    /** v1 syncs a single active profile; multi-profile editing is out of scope. */
    val activeProfileId: String = "user"

    private suspend fun db(): BountywellDb = mutex.withLock {
        database ?: run {
            val db = BountywellDb(driverFactory.create())
    ...
    private suspend fun migrateLegacyIfNeeded(db: BountywellDb) {
        val queries = db.bountywellQueries
    ...
    suspend fun loadState(): SyncState = withContext(dispatcher) {
        val row = db().bountywellQueries.selectState().awaitAsOneOrNull()
        SyncState(
    ...
    suspend fun writeState(blob: SyncBlob, syncedUpdatedAt: String?, dirty: Boolean) = withContext(dispatcher) {
        db().bountywellQueries.upsertState(
